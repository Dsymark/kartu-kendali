...
import { Plus, X, Car, Bike, AlertCircle, Sparkles } from "lucide-react";
...
export default function ModalTambahMobil({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.success) {
      ...
      setError(res.error || "Gagal menyimpan kendaraan baru.");
    }
  }
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-slate-900 to-indigo-950 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-blue-600/30 border border-blue-400/40 text-blue-300 flex items-center justify-center font-bold">
              {formData.jenis === "Motor" ? <Bike className="w-5 h-5" /> : <Car className="w-5 h-5" />}
            </div>
            <div>
              <h3 className="font-extrabold text-base tracking-tight">Daftarkan Kendaraan Dinas Baru</h3>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, jenis: e.target.value, tipe: e.target.value === "Motor" ? "Skuter" : "SUV" })}
